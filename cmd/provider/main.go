...
import (
	...
	"github.com/pelotech/provider-freeipa/apis"
	"github.com/pelotech/provider-freeipa/apis/v1alpha1"
	"github.com/pelotech/provider-freeipa/config"
	"github.com/pelotech/provider-freeipa/internal/clients"
	"github.com/pelotech/provider-freeipa/internal/controller"
	"github.com/pelotech/provider-freeipa/internal/features"
)

func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for FreeIPA").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-freeipa"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-freeipa",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add FreeIPA APIs to scheme")
	...
	kingpin.FatalIfError(controller.Setup(mgr, o), "Cannot setup FreeIPA controllers")
